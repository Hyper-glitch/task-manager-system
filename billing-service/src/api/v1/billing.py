...
class BillingCycle(Resource):
    ...
    def get(self) -> Response:
        ...
        try:
            billing_cycle = billing_service.get_billing_cycle(
                billing_cycle_id=data["billing_cycle_id"]
            )
        except BillingCycleNotFound:
            return Response(
                status=HTTPStatus.NOT_FOUND, response="Billing cycle not found"
            )
        else:
    ...
    def get_balances(self) -> Response:
        ...
        return make_response(
            code=200, **{"balance": balance, "pagination_count": count}
        )
